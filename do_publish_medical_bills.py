#!/usr/bin/env python3
"""
DO Console Script: Publish Medical Bills Article
Run this directly on DigitalOcean App Console:
    python do_publish_medical_bills.py
"""
...
# Get DATABASE_URL from environment (automatically set in DO)
...
ARTICLE_CONTENT = '''<h2>Why Your Hospital Bill Might Be Higher Than It Should Be</h2>
...
def publish():
    ...
    cur.execute("SELECT id FROM articles WHERE slug = %s", ('medical-bills-nepal-common-overcharges',))
    if cur.fetchone():
        print("Article already exists. Skipping.")
        conn.close()
        return

    # Get category ID for 'General Health' or 'Patient Rights'
    cur.execute("SELECT id FROM article_categories WHERE name IN ('Patient Rights', 'General Health') LIMIT 1")
    result = cur.fetchone()
    if not result:
        # Create category
        cur.execute("""
            INSERT INTO article_categories (name, slug, description, display_order)
            VALUES ('Patient Rights', 'patient-rights', 'Understanding your rights as a patient in Nepal', 1)
            RETURNING id
        """)
        category_id = cur.fetchone()[0]
        print(f"Created category 'Patient Rights' with ID: {category_id}")
    else:
    ...
    """, (
        'Understanding Medical Bills in Nepal: Common Overcharges to Watch For',
        'medical-bills-nepal-common-overcharges',
    ...
    print(f"   URL: /health-digest/medical-bills-nepal-common-overcharges")
